...
echo "Releasing ATalk SDK ${SDK_VERSION}"
...
# Now build and publish the ATalk SDK and its dependencies
echo "Building and publishing the ATalk SDK"
pushd ${THIS_DIR}/../
./gradlew clean 
...
if [[ $DO_GIT_TAG == 1 ]]; then
    ...
    pushd ${MVN_REPO_PATH}
    git add -A .
    git commit -m "ATalk SDK + dependencies: ${SDK_VERSION}"
    popd
